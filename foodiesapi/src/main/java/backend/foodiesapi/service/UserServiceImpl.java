...
public class UserServiceImpl implements UserService {
    ...
    private UserEntity convertToEntity(UserRequest request) {
        backend.foodiesapi.enums.Role userRole = backend.foodiesapi.enums.Role.CUSTOMER;
        if (request.getRole() != null && !request.getRole().isEmpty()) {
            try {
                userRole = backend.foodiesapi.enums.Role.valueOf(request.getRole().toUpperCase().replace("ROLE_", ""));
            } catch (IllegalArgumentException e) {
                userRole = backend.foodiesapi.enums.Role.CUSTOMER;
            }
        }
