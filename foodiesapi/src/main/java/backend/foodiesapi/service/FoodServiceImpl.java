...
public class FoodServiceImpl implements FoodService {
    ...
    public FoodResponse addFood(FoodRequest request, MultipartFile file) {
        ...
        // If it's a restaurant owner, try to use their restaurant ID
        if (authorizationUtil.isRestaurantOwner()) {
            ...
            if (ownerRestaurantId != null) {
                ...
                // Fallback for development/testing if owner has no restaurant linked yet
                // Assuming ID 1 exists or just to bypass the crash
                log.warn("Owner has no restaurant linked. Defaulting to ID 1 for testing.");
                newFood.setRestaurantId(1L);
            }
        } else if (request.getRestaurantId() != null) {
            newFood.setRestaurantId(request.getRestaurantId());
        } else {
            // Default to 1 if nothing provided (Development convenience)
            newFood.setRestaurantId(1L);
        }
    ...
    public boolean deleteFile(String filename) {
        // Cloudinary delete logic could be implemented here if we store public_id
        // For now, returning true as we are not managing local files
        return true;
    }
    ...
    private FoodResponse convertToResponse(FoodEntity entity) {
        log.debug("Converting FoodEntity ID: {} with RestaurantId: {} to response", entity.getId(),
